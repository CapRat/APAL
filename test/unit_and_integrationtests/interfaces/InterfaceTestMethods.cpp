...
void testIInfoComponent(XPlug::IInfoComponent* infoComp) {
    ...
    REQUIRE_MESSAGE(!infoComp->getPluginCopyright().empty(), "plugin copyright is empty");
    REQUIRE_MESSAGE(!infoComp->getPluginDescription().empty(), "plugin description is empty");
...
{
    REQUIRE_MESSAGE(port != nullptr, "Error, dont get nullptr, if size says there should be sth");
    REQUIRE_MESSAGE((port->getDirection() == PortDirection::Input || port->getDirection() == PortDirection::Output), "Ports can either be direction input or output. Nothing else at the moment.");
    REQUIRE_MESSAGE(!port->getPortName().empty(), "Give your Port a name.. .At least an non null str");
    switch (port->getType()) {
        case PortType::Audio:  testIAudioPort(dynamic_cast<IAudioPort*>(port));                     break;
        case PortType::MIDI:   testIMidiPort(dynamic_cast<IMidiPort*>(port));                       break;
        case PortType::None:   REQUIRE_MESSAGE(false, "There should be no use of None, right now"); break;
    default:  REQUIRE_MESSAGE(false, "No implemented version of PortType is used.");
    }
}
