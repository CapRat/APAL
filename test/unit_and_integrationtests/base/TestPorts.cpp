...
char
compareMidiMessage(MidiMessage a, MidiMessage b)
{
  size_t i;
...
TEST_CASE("Test Ports")
{
  ...
  REQUIRE_MESSAGE(compareMidiMessage(mPort.peek(), { 0x1, 0x2, 0x3 }),
                  "Queue not working properly");
  REQUIRE_MESSAGE(compareMidiMessage(mPort.get(), { 0x1, 0x2, 0x3 }),
                  "Queue not working properly");
  REQUIRE_MESSAGE(compareMidiMessage(mPort.get(), { 0x2, 0x3, 0x4 }),
                  "Queue not working properly");
}