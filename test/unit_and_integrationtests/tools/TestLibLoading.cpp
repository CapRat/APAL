/**
 * This file contains multiple Sources, which are controlled by preprocessor
 * define. This other Sources are needed for the Unittest. If no BUILD_TEST_LIB
 * preprocessor directive is defined, the normal Unittest is build. if its
 * defined 2 libraries can be build. To build the static library define
 * BUILD_STATIC_EXAMPLE and to build  tge simple symbol export define
 * BUILD_SIMPLE_SYMBOL_EXPORT.
 */
...
TEST_CASE("Symbolload checking", "[symbol]")
{
  ...
  {
    auto static_test_function =
      LoadFunc<int (*)(int)>(lib, "static_test_function");
    REQUIRE_MESSAGE(static_test_function != nullptr, GetErrorStr());
    REQUIRE(static_test_function(2) == 2 + 5);
    // Load Hidden function
    auto static_test_function2 =
      LoadFunc<int (*)(int)>(lib, "static_test_function2");
    REQUIRE_MESSAGE(static_test_function2 != nullptr, GetErrorStr());
    REQUIRE(static_test_function2(2) == 2 + 5);
  }
}
#else
/**Static testlibrary, to link this static function in an shared library and
 * check if function is inside of it**/
...
extern "C"
{
  int static_test_function2(int x) { return 5 + x; }
}
#endif
...
extern "C"
{
  ...
  int SIMPLE_SYMBOL_EXPORT_EXPORT test_function() { return 2; }
}
