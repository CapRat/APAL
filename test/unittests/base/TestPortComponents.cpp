...
TEST_CASE("Test DynamicPortComponent") {
	...
	DynamicPortComponent initFillDynComp { std::make_shared<MonoPort>("In0", PortDirection::Input),
		                                   std::make_shared<MonoPort>("Out0", PortDirection::Output) };
	DynamicPortComponent emptyDynComp;

	//testing, weather Monoport does, what it should do. If it does, continue with this tests.
	auto mPort = MonoPort("In0", PortDirection::Input);
	REQUIRE_MESSAGE(mPort.getPortName() == "In0", "The Port, works not properly. No PortComponenttests can be executed");
	testIPort(&mPort);
	...
	dynComp.addPort(std::make_unique<QueueMidiPort>("MidiIn0", PortDirection::Input));
	dynComp.addPort(std::make_unique<QueueMidiPort>("MidiIn1", PortDirection::Input));
	dynComp.addPort(std::make_unique<QueueMidiPort>("MidiOut0", PortDirection::Output));
