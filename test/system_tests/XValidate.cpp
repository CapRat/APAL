...
void
RegisterTestSuite(std::shared_ptr<IFormatTestSuite> testSuite)
{
...
main(int argc, char* argv[])
{
  ...
  auto cli =
    (option("-p", "--performance")
       .set(enablePerformanceMeasureing)
       .doc("Enables performancemeasurement of functions."),
     option("-w", "--verbose")
       .set(verboseLogging)
       .doc("Prints all available messages."),
     option("-q", "--quiet")
       .set(silentLogging)
       .doc("Prints nothing, except errors."),
     option("-nt", "--notest")
       .set(disableTesting)
       .doc("Dont run regular tests."),
     option("-l", "--strictnessLevel")
         .doc("Sets the Level, how strict the tests should be executed.") &
       value("strictness level", strictnessLevel));
  for (auto testSuite : GlobalData().testSuites) {
    auto formatName = testSuite->getFormatName();
    std::transform(
      formatName.begin(), formatName.end(), formatName.begin(), ::tolower);
    cli.push_back(
      option("-" + formatName, "--enable" + testSuite->getFormatName())
        .call([testSuite, &testDefaultSelection](const char*) {
          testSuite->enable();
  ...
  TestSuiteData tData{ pluginPath,
                       (verboseLogging
                          ? VerbosityLevel::Verbose
                          : silentLogging ? VerbosityLevel::Quiet
                                          : VerbosityLevel::Normal),
                       strictnessLevel };

  // Initialize all testsuites and enable or disable them, if default selection
  // is needed.
  for (auto testSuite : GlobalData().testSuites) {
  ...
  for (auto testSuite : GlobalData().testSuites) {
    if (testSuite->isEnabled()) {
      ...
      if (!disableTesting) {
        GlobalLog().logN("Execute " + testSuite->getFormatName() +
                           " Tests with strictness " +
                           std::to_string(strictnessLevel),
                         LoggerValue::INFO);
        testSuite->run();
      }

      if (enablePerformanceMeasureing) {
        GlobalLog().logN("Execute Performanceanalysis for " +
                           testSuite->getFormatName() + " Format",
                         LoggerValue::INFO);
      }
    }
  }
