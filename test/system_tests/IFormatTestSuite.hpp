...
public:
  ...
  /**
   * @brief Initializes the Testsuite with needed data. Do heave initialisation
   * in here.
  ...
  /**
   * @brief Is needed, when the automatic detection of formats is enabled. This
   * Function indicates, weather the given testobjects, meet the requirements to
   * be tested or not. This Function is called before initialize is called.
   * @return
   */
  ...
  /**
   * @brief returns true, if the TestSuite is enabled, false if the Suite is
   * disabled.
   * @return returns true, if the TestSuite is enabled, false if the Suite is
   * disabled.
   */
   ...
   * @param strictnessLevel The Higher the StrictnessLevel, the harder the tests
   * are to pass.
...
#define REGISTER_TEST_SUITE(TestClassName)                                     \
  static TestSuiteRegistrator<TestClassName> instance##TestClassName =         \
    TestSuiteRegistrator<TestClassName>()
...
public:
  ...
  inline virtual SucceedState test(int strictnessLevel,
                                   bool cond,
                                   std::string failMsg)
  {
