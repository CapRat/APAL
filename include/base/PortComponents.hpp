...
/*  template<size_t inputPortsSize, size_t outputPortsSize>
  ...
  public:
      StaticPortComponent(std::array<std::unique_ptr<IPort>, inputPortsSize>
  inputs, std::array<std::unique_ptr<IPort>, outputPortsSize> outputs) {
          this->inputPorts = std::move(inputs);
          this->outputPorts = std::move(outputs);
      }
      // Geerbt über IPortComponent
      inline virtual size_t size() override { return inputPortsSize +
  outputPortsSize; } inline virtual IPort* at(size_t index) override { return
  index < inputPortsSize ? inputPorts[index].get() : outputPorts[index -
  inputPortsSize].get; } inline virtual size_t sizeInputPorts() { return
  inputPortsSize; } inline virtual IPort* inputPortAt(size_t index) { return
  inputPorts[index].get(); } inline virtual size_t sizeOutputPorts() { return
  outputPortsSize; } inline virtual IPort* outputPortAt(size_t index) { return
  outputPorts[index].get(); } protected: std::array<std::unique_ptr<IPort>,
  inputPortsSize> inputPorts; std::array<std::unique_ptr<IPort>,
  outputPortsSize> outputPorts;
  };*/
}
