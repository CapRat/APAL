...
extern "C"
{
  IPluginFactory* PLUGIN_API GetPluginFactory()
  {
    if (!gPluginFactory) {
      ...
      gPluginFactory->registerClass(&componentClass, [](void*) -> FUnknown* {
        return (IAudioProcessor*)new VST3AudioProccessorImpl();
      });
      static std::string controllerName =
        std::string(plug->getInfoComponent()->getPluginName()) + "Controller";
      ...
      gPluginFactory->registerClass(
        &componentContollerClass, [](void*) -> FUnknown* {
          return (IEditController*)new VST3EditControllerImpl();
        });
    } else
