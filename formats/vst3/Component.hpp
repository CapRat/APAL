...
/**
 * @brief Implementation of the VST IComponent interface.
 */
...
public:
  ...
  /**
   * @brief The host passes a number of interfaces as context to initialize the
   * Plug-in class.
   ...
   * kResultOk, the object is released immediately. In this case terminate is
   * not called!
   */
  virtual tresult PLUGIN_API initialize(FUnknown* context) override;
  /**
   * @brief  This function is called before the Plug-in is unloaded and can be
   * used for cleanups. You have to release all references to any host
   * interfaces.
   */
  virtual tresult PLUGIN_API terminate() override;
  /**
   * @brief Called before initializing the component to get information about
   * the controller class. */
  virtual tresult PLUGIN_API getControllerClassId(TUID classId) override;
  /**
   * @brief Called before 'initialize' to set the component usage (optional).
   * See \ref IoModes
   */
  virtual tresult PLUGIN_API setIoMode(IoMode mode) override;
  /**
   * @brief Called after the Plug-in is initialized. See \ref MediaTypes,
   * BusDirections
   */
  ...
  /**
   * @brief Retrieves routing information (to be implemented when more than one
  ...
  /**
   * @briefCalled upon (de-)activating a bus in the host application. The
   * Plug-in should only processed an activated bus, the host could provide less
   * see \ref AudioBusBuffers in the process call (see
   * \refIAudioProcessor::process) if last buses are not activated */
