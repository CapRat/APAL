...
{
    if (type == kAudio) {
       return static_cast<int32>(XPlug::getNumberOfPorts<XPlug::IAudioPort>(XPlug::GlobalData().getPlugin(plugIndex).get(),
            dir == kInput ? XPlug::PortDirection::Input : XPlug::PortDirection::Output));
    }
    else if(type == kEvent){
        return  static_cast<int32>(XPlug::getNumberOfPorts<XPlug::IMidiPort>(XPlug::GlobalData().getPlugin(plugIndex).get(),
            dir == kInput ? XPlug::PortDirection::Input : XPlug::PortDirection::Output));
    }
    else {
        return 0;
...
tresult PLUGIN_API Component::getBusInfo(MediaType type, BusDirection dir, int32 index, BusInfo& bus)
{
    ...
    if (index < 0 || index > getBusCount(type, dir)) return kInvalidArgument;
    if (type == kAudio) {
        auto ap=XPlug::getPortAt<XPlug::IAudioPort>(XPlug::GlobalData().getPlugin(plugIndex).get(), static_cast<size_t>(index), dir == kInput ? XPlug::PortDirection::Input : XPlug::PortDirection::Output);
        p = ap;
        bus.channelCount = static_cast<int32>(ap->size());
    }
    else if (type == kEvent) {
        auto mp = XPlug::getPortAt<XPlug::IMidiPort>(XPlug::GlobalData().getPlugin(plugIndex).get(), static_cast<size_t>(index), dir == kInput ? XPlug::PortDirection::Input : XPlug::PortDirection::Output);
        p = mp;
        bus.channelCount = 16;
    }
    else{
