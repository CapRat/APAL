...
public:
  inline virtual Steinberg::int32 getEventCount() override
  {
    return (Steinberg::int32)events.size();
  }
  inline virtual ~EventList(){};
  inline virtual Steinberg::tresult getEvent(Steinberg::int32 index,
  ...
  // dont implement that stupid COM Interface, cause it should not be used
  // here...
  inline virtual Steinberg::tresult queryInterface(const Steinberg::TUID,
                                                   void**) override
  {
...
/**
 * @brief Simple class to handle allocating and freeing of Memory for
 * ProcessData struct.
 */
...
protected:
  ...
  static inline size_t getNumberOfChannel(
    Steinberg::Vst::SpeakerArrangement arr)
  {
    size_t number = 0;
    while (arr) {
      if (arr & (Steinberg::Vst::SpeakerArrangement)1)
        ++number;
      arr >>= 1;
    }
    return number;
...
public:
   ...
   * @param inputChannels number and Arrangements of inputchannels. The size
   * indicates, how much inputs are used.
   * @param outputChannels number and Arrangements of outputchannels. The size
   * indicates, how much outputs are used.
   * @param sampleSize size of on Sample. Values can be 32 or 64. Values from
   * SymbolicSampleSizes
   * @param numSamples Numbers of Samples in one buffer per bus.
   */
  inline ProcessDataClass(
    std::vector<Steinberg::Vst::SpeakerArrangement> _inputChannels,
    std::vector<Steinberg::Vst::SpeakerArrangement> _outputChannels,
    Steinberg::Vst::SymbolicSampleSizes _sampleSize,
    size_t _numSamples)
  {
    ...
    auto allocator = [this](std::vector<AudioBusBuffers>& buffers,
                            std::vector<SpeakerArrangement>& channelConfig) {
      for (size_t i = 0; i < buffers.size(); i++) {
        ...
        if (this->symbolicSampleSize == SymbolicSampleSizes::kSample32) {
          buffers[i].channelBuffers32 =
            new Sample32*[this->inputs[i].numChannels];
          ...
          buffers[i].channelBuffers64 =
            new Sample64*[this->inputs[i].numChannels];
          for (int j = 0; j < buffers[i].numChannels; j++) {
...
#define WARN_IF_AND_THAN(cond, msg, andthen)                                   \
  if (cond) {                                                                  \
    lastWarning = msg;                                                         \
    andthen                                                                    \
  }
#define WARN_IF(cond, msg) WARN_IF_AND_THAN(cond, msg, return false;)
#define ERROR_IF_AND_THAN(cond, msg, andthen)                                  \
  if (cond) {                                                                  \
    lastError = msg;                                                           \
    andthen                                                                    \
  }
...
public:
  ...
  inline bool hasInitAndExitFunctions()
  {
  ...
  inline explicit VST3Module(std::string pluginPath)
  {
    ...
    ERROR_IF_AND_THAN(this->pluginLibrary == nullptr,
                      "Error, while loading VST3 binary, with Message :" +
                        GetErrorStr(),
                      return;)

    this->plugLoadFunction =
      LoadFunc<GetFactoryProc>(this->pluginLibrary, "GetPluginFactory");
    ERROR_IF_AND_THAN(this->plugLoadFunction == nullptr,
                      "Error, while loading VST3 function, with Message :" +
                        GetErrorStr(),
                      return;)
#if defined(WIN32)
    this->initfnc = LoadFuncRaw(this->pluginLibrary, "InitDll");
    this->exitfnc = LoadFuncRaw(this->pluginLibrary, "ExitDll");
    this->runInitFncLampda = [this]() -> bool {
      return ((bool (*)())this->initfnc)();
    };
    this->runExitFncLampda = [this]() -> bool {
      return ((bool (*)())this->exitfnc)();
    };
#elif defined(__APPLE__)
    this->initfnc = LoadFuncRaw(this->pluginLibrary, "bundleEntry");
    this->exitfnc = LoadFuncRaw(this->pluginLibrary, "bundleExit");
    this->runInitFncLampda = [this]() -> bool {
      return ((bool (*)(CFBundleRef))this->initfnc)();
    };
    this->runExitFncLampda = [this]() -> bool {
      return ((bool (*)())this->exitfnc)();
    };
    ...
    this->runInitFncLampda = [this]() -> bool {
      return ((bool (*)(void*))this->initfnc)((void*)pluginLibrary);
    };
    this->runExitFncLampda = [this]() -> bool {
      return ((bool (*)())this->exitfnc)();
    };
#else
    assert("Warning, cant determinate Sytem. No Initfunction(like InitDll) is "
           "executed nor testeted");
#endif //! WIN32
    WARN_IF_AND_THAN(hasInitAndExitFunctions(),
                     "No initialisation functions given, for VST3 Module." +
                       GetErrorStr(),
                     return;);
  ...
  inline bool initialize()
  {
    ...
    ERROR_IF(this->factory == nullptr,
             "Error while loading factory. Factory is NULL");
    ERROR_IF((this->factory->countClasses() < 2),
             "Error There are less then 2 classes in Factory.There must be at "
             "least one IAudioProcessor and one IEditController class.");
    PFactoryInfo info;
    ERROR_IF((this->factory->getFactoryInfo(&info) != kResultOk),
             "Cant get FactoryInfo from factory");
    ...
    for (Steinberg::int32 i = 0; i < this->factory->countClasses(); i++) {
      ...
      if (factory3 != nullptr) {
        ...
        ERROR_IF((this->factory->getFactoryInfo(&facInfo) != kResultOk),
                 "Cant get FactoryInfo from factory");
        Steinberg::PClassInfo2 ci2(ci.cid,
                                   ci.cardinality,
                                   ci.category,
                                   ci.name,
                                   0,
      ...
      if (strcmp(ci3.category, kVstAudioEffectClass) ==
          0) { // Class is an Audio Effect Processor
        ERROR_IF(
          ((this->factory->createInstance(
              ci3.cid, Vst::IComponent::iid, (void**)&processorComponent) !=
            kResultOk)),
          "Error, could not create IComponentobject successfully");
        std::string x;
        int j = 0;
        while (ci3.name[j] != 0) {
          x.push_back(static_cast<char>(ci3.name[j++]));
        }
        ERROR_IF((processorComponent->initialize(nullptr) != kResultOk),
                 std::string("Error while initialising Component ") + x);
        TUID controllerCID;
        ERROR_IF(processorComponent->getControllerClassId(controllerCID) !=
                   kResultTrue,
                 "Error,while getting ControllerID from IComponent.");
        auto controllerFUID = FUID::fromTUID(controllerCID);

        ERROR_IF(!controllerFUID.isValid(),
                 "Error, ControllerID from IComponent is not valid.");
        ERROR_IF(
          (this->factory->createInstance(controllerCID,
                                         Vst::IEditController::iid,
                                         (void**)&editController) != kResultOk),
          "Error while creating EditController from Factory.");
        ERROR_IF(editController->initialize(nullptr) != kResultOk,
                 "Error while initializing EditController");
        ERROR_IF((processorComponent->queryInterface(Vst::IAudioProcessor::iid,
                                                     (void**)&audioProcessor) !=
                  kResultOk),
                 "Error, while retreiving the AudioInterface");
      } else {
        continue;
      }
    }
  ...
  inline bool deinitalize()
  {
    if (editController != nullptr)
      ERROR_IF(editController->terminate() != Steinberg::kResultOk,
               "Error while terminating IEditController implementation.");
    if (processorComponent != nullptr)
      ERROR_IF(processorComponent->terminate() != Steinberg::kResultOk,
               "Error while terminating IEditController implementation.");
    return true;
  }
  ...
  inline bool allocateData(size_t audioPortBufferSize)
  {
    using namespace Steinberg;
    using namespace Vst;
    std::vector<SpeakerArrangement> inputs(
      this->processorComponent->getBusCount(MediaTypes::kAudio,
                                            BusDirections::kInput));
    std::vector<SpeakerArrangement> outputs(
      this->processorComponent->getBusCount(MediaTypes::kAudio,
                                            BusDirections::kOutput));
    SymbolicSampleSizes size =
      this->audioProcessor->canProcessSampleSize(kSample64) ? kSample64
                                                            : kSample32;
    size_t i = 0;
    for (auto& sArr : inputs)
      ERROR_IF(this->audioProcessor->getBusArrangement(
                 BusDirections::kInput, i++, sArr) != kResultOk,
               "Error, Couldnt get Busarrangement");
    i = 0;
    for (auto& sArr : outputs)
      ERROR_IF(this->audioProcessor->getBusArrangement(
                 BusDirections::kOutput, i++, sArr) != kResultOk,
               "Error, Couldnt get Busarrangement");
    pData = std::make_unique<ProcessDataClass>(
      inputs, outputs, size, audioPortBufferSize);
    return true;
  }
  ...
  inline bool run()
  {
    ERROR_IF(this->audioProcessor->setProcessing(true) != Steinberg::kResultOk,
             "Error, while set Processing to true");
    ERROR_IF(this->audioProcessor->process(*(this->pData.get())) !=
               Steinberg::kResultOk,
             "Error, because processing not returned valid State");
    ERROR_IF(this->audioProcessor->setProcessing(false) != Steinberg::kResultOk,
