...
static void *
dlopenLADSPA(const char * pcFilename) {
  ...
  if (pcFilename[0] == '/') {
    ...
    /* The filename is absolute. Assume the user knows what he/she is
       doing and simply XPlug_LoadLibrary() it. */

    pvResult = XPlug_LoadLibrary(pcFilename);
    if (pvResult != NULL)
      return pvResult;
    ...
    while (*pcStart != '\0') {
      ...
      pvResult = XPlug_LoadLibrary(pcBuffer);
  ...
  if (!iEndsInSO) {
    ...
    strcat(pcBuffer, ".so");
    pvResult = dlopenLADSPA(pcBuffer);
    free(pcBuffer);
  }
  ...
  /* If nothing has worked, then at least we can make sure we set the
     ...
     dlopen() again here. */
  return XPlug_LoadLibrary(pcFilename);
}
...
loadLADSPAPluginLibrary(const char * pcPluginFilename) {
  ...
  pvPluginHandle = dlopenLADSPA(pcPluginFilename);
  if (!pvPluginHandle) {
    fprintf(stderr, 
...
unloadLADSPAPluginLibrary(void * pvLADSPAPluginLibrary) {
  XPlug_CloseLibrary(pvLADSPAPluginLibrary);
}
...
findLADSPAPluginDescriptor(void * pvLADSPAPluginLibrary,
  ...
  pfDescriptorFunction 
    = (LADSPA_Descriptor_Function)XPlug_LoadFunction(pvLADSPAPluginLibrary,
					"ladspa_descriptor");
  if (!pfDescriptorFunction) {
    const char * pcError = XPlug_LibError();
    if (pcError) {
      fprintf(stderr,
      ...
      exit(1);
        ...
        fprintf(stderr, "fouuuund"); exit(1);
    }
  }
